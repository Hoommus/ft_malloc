...
__attribute__((constructor,used)) static void	malloc_init(void)
{
	...
	region_create_zone(g_storage->regions + 0, BLK_TINY, ALIGN_TO_PAGE(REGION_TINIES_SIZE, pagesize));
	region_create_zone(g_storage->regions + 1, BLK_SMALL, ALIGN_TO_PAGE(REGION_SMALLIES_SIZE, pagesize));
}
...
void											*malloc(size_t size)
{
	...
	ft_putchar('\n');
//	write(1, "f_malloc has been called\n", 25);
	if (size == 0)
		return (NULL);
